...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // Obtener el token actual de Keycloak (asíncrono) sólo cuando estemos logueados
    return from(this.keycloakService.getToken()).pipe(
      ...
      // Si al obtener token hay un error, evitamos forzar login
      catchError((e) => {
        console.warn('[AuthInterceptor] Error getting token, proceeding without Authorization header', e);
        return next.handle(request);
      })
    );
  }
